...
export const founderBudgetBanners = {
  ...
  budget_rejected: {
    tone: "danger",
    message: "예산안이 반려되었습니다. 내용을 보완해 다시 제출해 주세요.",
  },
  budget_approved: {
    tone: "success",
  ...
  change_submitted: {
    tone: "info",
    message: "예산 변경 요청이 검토 중입니다. 검토가 완료될 때까지 지출 신청과 추가 변경 요청이 제한됩니다.",
    dismissible: true,
  },
  change_revision_requested: {
    tone: "warning",
    message: "예산 변경에 보완 요청이 있습니다. 검토 의견을 확인해 금액을 수정한 뒤 다시 제출해 주세요.",
    dismissible: true,
  },
  change_rejected: {
    tone: "danger",
    message: "예산 변경 요청이 반려되었습니다. 기존 확정 예산은 유지되며, 필요 시 다시 변경을 요청할 수 있습니다.",
    dismissible: true,
  },
  change_approved: {
    tone: "success",
    message: "예산 변경이 승인되어 반영되었습니다. 변경된 예산으로 지출 신청을 진행할 수 있습니다.",
    dismissible: true,
  },
};
...
export function getBudgetStatusTone(status) {
  ...
  if (["budget_rejected", "change_rejected"].includes(status)) return "danger";
...
// 최초 예산안이 한 번이라도 승인되면(이후 변경 요청 중/반려 포함) 기존 확정 예산으로 지출 가능하다.
